...
def load_and_select_env():
    ...
    if uploaded_env_file:
        try:
            ...
            if missing_keys or invalid_paths:
                ...
                if missing_keys:
                    error_message += f"Missing keys: {
                        ', '.join(missing_keys)}\n"
                if invalid_paths:
                    error_message += f"Invalid paths: {
                        ', '.join(invalid_paths)}"
    ...
    if st.session_state.get("create_env", False):
        ...
        omim_annotation_path = st.text_input(
            "OMIM annotation path:", "./resources/omim.txt"
        )
        candidate_list_dir = st.text_input(
            "Candidate list directory:", "./resources/candidate_lists"
        )
        reference_files_dir = st.text_input(
            "Reference files directory:", "./resources/references"
        )
        annotsv_format_path = st.text_input(
            "Annotsv format path:", "./resources/annotsv_format.txt"
        )
        ...
        if st.button("Save new `.env` file"):
            try:
                ...
                invalid_paths = [
                    path for path in paths.values() if not Path(path).exists()
                ]
                if invalid_paths:
                    raise ValueError(
                        f"Invalid paths: {
...
def main(env_file_path):
    ...
    selected_candidate = st.sidebar.radio(
        "Select the desired candidate gene list", candidate_options
    )
    ...
    if entered_cns and entered_cnr_new:
        ...
        fig_scatter = cnvlib.do_scatter(
            input_cnr,
            segments=input_cns,
            y_min=-2,
            y_max=2,
            show_range=None if selected_scatter == "All" else selected_scatter,
        )
