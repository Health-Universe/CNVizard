...
def save_tables_as_excel(total_df:pd.DataFrame,bintest_df:pd.DataFrame,hom_del_df:pd.DataFrame,total_candi_df:pd.DataFrame,
...
def save_filtered_table_as_excel(filtered_df:pd.DataFrame, filtered_name:str):
    """
    Function which writes the preset DataFrames to an excel file 
    each preset represents an excel sheet
    Input Arguments: 
...
def save_tables_as_excel_tsv(filtered_tsv:pd.DataFrame):
    """
